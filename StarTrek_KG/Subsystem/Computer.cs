...
namespace StarTrek_KG.Subsystem
{
    ...
    public class Computer : SubSystem_Base
    {
        ...
        public Computer(IShip shipConnectedTo): base(shipConnectedTo)
        {
        ...
        public override List<string> Controls(string command)
        {
            ...
            switch (command.ToLower())
            {
                case OldCommands.Computer.GalacticRecord:
                    Computer.For(this.ShipConnectedTo).PrintGalacticRecord(game.Map.Sectors);
                    break;

                case OldCommands.Computer.Status:

                    //todo: get a list of all baddie names in Sector

                    this.PrintCurrentStatus(game.Map, 
                                              this.Damage, 
                                              starship,
                                              this.ShipConnectedTo.GetSector());
                    break;
        ...
        public List<KeyValuePair<int, Coordinate>> ListObjectsInRegion()
        {
            ...
            if (this.Damaged())
            {
                this.ShipConnectedTo.OutputLine("Unable to List Objects in Sector");
                return list;
            }
            ...
            if (sectorsWithObjects.Any())
            {
                ...
                foreach (var sector in sectorsWithObjects)
                {
                    ...
                    list.Add(new KeyValuePair<int, Coordinate>(objectNumber, sector));
            ...
            {
                this.ShipConnectedTo.OutputLine("No Coordinates with Objects found (this is an error)");
            }
        ...
        private void PrintCurrentStatus(IMap map, int computerDamage, IShip ship, ISector currentRegion)
        {
            ...
            //todo: Display all baddie names in Sector when encountered.
        }

        //This needs to be output as an array, List<List>, or KeyValueCollection, and the grid needs to be generated by the UI app

        private void PrintGalacticRecord(IReadOnlyCollection<Sector> Sectors)
        {
            if (this.Damaged()) return;
        ...
        private void RenderScannedRegion(ISector Sector, Location myLocation, int RegionUB, int RegionLB)
        {
            ...
            {
                starbaseCount = Sector.GetStarbaseCount();
                starCount = Sector.GetStarCount();
                hostileCount = Sector.GetHostiles().Count;
            }
        ...
        public new static Computer For(IShip ship)
        {
